...
#base_dir = 'input/input/classed_upload' #for tests
...
features = np.zeros(shape=(bathroom_input_cnt,10,10, 2048))
...
pickle_out = open("images_classed_df/df_features_bathroom.pickle","wb")
...
for inputs_batch in tqdm(generator):
    try:
        ...
        i += 1
        if i * batch_size >= bathroom_input_cnt:
...
features = np.reshape(features, (bathroom_input_cnt, 10 * 10 * 2048))
print('done reshaping and now saving')
np.save('images_classed_arry/arry_features_bathroom.npy', features)
print('done')